...
The API has a "v1", and the layout also has a "v1" and a "v2". This is unrelated. The "v1" API will continue to be used, even when the "v2" layout goes live.
...
All languages are stored in `resources/lang`. The main language is `en_US`, and the translations are derived from these English sentences. Any language in this directory is at least 75% complete, as measured by Crowdin. If you want to support the translations, check out [the Crowdin project](https://crowdin.com/project/firefly-iii).